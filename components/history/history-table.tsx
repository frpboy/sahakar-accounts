            ...
            <div className="flex flex-col md:flex-row justify-between items-center gap-4 bg-white p-4 rounded-xl border shadow-sm">
                <div className="relative w-full md:w-96">
                    ...
                    <input
                        ...
                        className="w-full pl-10 pr-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 bg-gray-50/50"
                        value={searchTerm}
                ...
                <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
                    <div className="flex items-center gap-2 bg-gray-50 px-3 py-1.5 rounded-lg border text-sm">
                    ...
                    <button
                        ...
                        className="flex items-center gap-2 px-4 py-2 bg-gray-900 text-white rounded-lg text-sm font-medium hover:bg-gray-800 transition-colors shadow-sm active:scale-95"
                    >
            ...
            <div className="bg-white rounded-xl shadow-sm border overflow-hidden">
                    ...
                    <table className="w-full text-left">
                        <thead className="bg-gray-50 border-b">
                            <tr>
                                ...
                                <th className="px-6 py-4 text-xs font-semibold text-gray-500 uppercase tracking-wider">Internal ID</th>
                                ...
                                filteredData.map((t) => (
                                    ...
                                    >
                                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                                            <div className="font-medium text-gray-900">{format(new Date(t.created_at), 'dd MMM yyyy')}</div>
                                            <div className="text-[10px] text-gray-400">{format(new Date(t.created_at), 'hh:mm a')}</div>
                                        </td>
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            <span className="text-sm font-mono font-bold text-gray-900 bg-gray-50 px-2 py-1 rounded border border-gray-100">
                                                {t.internal_entry_id || '---'}
                                            </span>
                                        </td>
                                        ...
                                        <td className="px-6 py-4 text-sm text-gray-600">
                                            ...
                                            <div className="text-[10px] text-gray-400 uppercase tracking-tighter flex items-center gap-2">
                                                {t.payment_modes?.split(',').map((m: string) => (
                                                    <span key={m} className="bg-gray-100 px-1 rounded">{m}</span>
                                                ))}
                                            </div>
                                        </td>
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            <div className="flex items-center gap-2">
                                                {t.daily_records?.status === 'locked' ? (
                                                    <span className="flex items-center gap-1.5 px-2 py-1 rounded-full bg-red-50 text-red-600 text-[10px] font-bold uppercase border border-red-100">
                                                        <Lock className="w-3 h-3" /> Locked
                                                    </span>
                                                ) : (
                                                    <span className="flex items-center gap-1.5 px-2 py-1 rounded-full bg-green-50 text-green-600 text-[10px] font-bold uppercase border border-green-100">
                ...
                <div className="px-6 py-4 bg-gray-50 border-t flex items-center justify-between">
                    <p className="text-xs text-gray-500">
                        Showing <span className="font-bold text-gray-700">{filteredData.length}</span> entries
                    </p>
                    <div className="flex gap-2">
                        <button disabled className="p-1 border rounded bg-white text-gray-300 cursor-not-allowed">
